...
/// Creates a new `ParentUpdateSystem`.
pub fn parent_update_system() -> impl Runnable {
    // Entities with a removed `Parent`
    let removed_parent_query = <(Entity, &PreviousParent)>::query().filter(!component::<Parent>());

    // Entities with a changed `Parent`
    let changed_parent_query = <(Entity, &Parent, &mut PreviousParent)>::query()
        .filter(component::<Transform>() & maybe_changed::<Parent>());

    // Deleted Parents (i.e. Entities with `Children` and without a `Transform`)
    let deleted_parent_query = <(Entity, &Children)>::query().filter(!component::<Transform>());

    SystemBuilder::<()>::new("ParentUpdateSystem")
        .write_component::<Children>()
        .with_query(removed_parent_query)
        .with_query(changed_parent_query)
        .with_query(deleted_parent_query)
        .build(|commands, world, _, queries| {
            let (removed_parent_query, changed_parent_query, deleted_parent_query) = queries;
            ...
            let (mut left, mut right) = world.split::<&mut Children>();
